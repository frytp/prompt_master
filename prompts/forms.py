...
class PromptForm(forms.ModelForm):
    ...
    def clean_title(self):
        ...
        if len(title_value) < MIN_TITLE_LENGTH:
            raise ValidationError(
                ...
                f'(минимум {MIN_TITLE_LENGTH} символа)'
            )
    ...
    def clean_content(self):
        ...
        if len(content_value) < MIN_CONTENT_LENGTH:
            raise ValidationError(
                f'Текст промпта слишком короткий '
                f'(минимум {MIN_CONTENT_LENGTH} символов)'
            )
    ...
    def save(self, commit=True):
        ...
        new_collection_name = self.cleaned_data.get(
            'new_collection', ''
        ).strip()
        if new_collection_name:
            collection, _ = Collection.objects.get_or_create(
                ...
                defaults={
                    'description': f'Коллекция {new_collection_name}',
                    'color': '#9b59b6'
                }
            )
        ...
        if new_models_str:
            new_model_names = [
                m.strip() for m in new_models_str.split(',') if m.strip()
            ]
...
class ImportPromptsForm(forms.Form):
    ...
    def clean_json_file(self):
        ...
        try:
            ...
            raise ValidationError(
                'Ошибка кодировки файла. Используйте UTF-8'
            ) from exc
